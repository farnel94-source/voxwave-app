...
- **Windows: opaque rectangle around the orb after Settings dialog close.** PySide6 6.11+ silently drops the `WS_EX_LAYERED` flag on certain `show()` calls (notably `_check_orb_health()` re-show after Settings dialog took focus). `OrbWidget` now re-applies `_setup_layered_window()` + `_render_layered()` on every `showEvent`. Fixes the gray 300x116 rectangle that started appearing around the logo.
- **Hotkey mode: orb stayed visible after each dictation.** `sig_hide_widget_delayed` was declared `Signal(str)` instead of `Signal()`, so `emit()` without args raised a silent `TypeError` in Qt and the deferred hide never ran. The orb now correctly disappears ~900ms after each transcription completes in `activation_method = "hotkey"` mode.
